...
else{ig.xhr('/api/',{load:true},this.receivedData.bind(this));}},receivedData:function(response){this.data=response.games;this.maxScore=0;for(var i=0;i<this.data.length;i++){var d=this.data[i];d.created=new Date(d.created*1000);this.maxScore=Math.max(this.maxScore,d.score);}
...
ig.baked=true;ig.module('game.menus.title').requires('game.menus.base','game.menus.settings','game.menus.detailed-stats','game.ease').defines(function(){MenuItemNormalMode=MenuItem.extend({getText:function(){return'new game';},ok:function(){ig.game.setGame();},});MenuItemSettingsMenu=MenuItem.extend({getText:function(){return'settings';},ok:function(){ig.game.menu=new MenuSettings();},});MenuItemChangeURL=MenuItem.extend({getText:function(){return(ig.doc?'try a different text':'Own text loading doesnt work');},ok:function(){document.location.href='/i_said_it_dont_work'},});MenuItemRemoveAds=MenuItem.extend({getText:function(){return'remove ads';},ok:function(){ig.game.purchaseRemoveAds();}});MenuItemGameCenter=MenuItem.extend({getText:function(){return'gamecenter';},ok:function(){ig.game.gameCenter.showLeaderboard('score');}});MenuItemStats=MenuItem.extend({getText:function(){return'Stats dont work';},ok:function(){ig.game.detailedStats=ig.game.detailedStats||new DetailedStats();ig.game.detailedStats.show();ig.input.delayedKeyup.click=true;}});MenuItemAppStore=MenuItem.extend({getText:function(){return'download as app';},ok:function(){window.location.href=ig.ua.iOS?'https://goo.gl/qSntlx':'https://goo.gl/cg7rmI';}});MenuTitle=Menu.extend({itemClasses:[MenuItemNormalMode,MenuItemRemoveAds,MenuItemGameCenter],scale:0.75,y:0,init:function(){this.itemClasses=[];this.itemClasses.push(MenuItemNormalMode);this.itemClasses.push(MenuItemSettingsMenu);if(window.Ejecta&&!ig.storageGet('removeAds')){this.itemClasses.push(MenuItemRemoveAds);}
...
ig.baked=true;ig.module('game.entities.emp').requires('impact.entity').defines(function(){"use strict";window.EntityEMP=ig.Entity.extend({lifetime:1,fadetime:1,alpha:0,killedProjectiles:false,img:new ig.Image('media/sprites/emp.png'),sound:new ig.Sound('media/sounds/emp.ogg',false),init:function(x,y,settings){this.parent(x,y,settings);this.idleTimer=new ig.Timer();this.sound.play();ig.game.screenShake(30);},update:function(){this.alpha=this.idleTimer.delta().map(this.lifetime-this.fadetime,this.lifetime,1,0);var scale=this.alpha.map(0,1,1.7,0);scale=Math.pow(scale,0.5)*200;var ents=ig.game.entities;for(var i=0;i<ents.length;i++){var ent=ents[i];if(ent instanceof EntityEnemy){if(this.distanceTo(ent)<scale){ent.spawnExplosionParticles(true);ent.kill();}}}
...
this.unsetHoverKeyTimer=new ig.Timer();},showMultiplier:function(m){this.multiplierTimer=new ig.Timer(2);this.multiplierIndex=m-2;this.multiSounds[m].play();},draw:function(){if(ig.ua.mobile){this.drawFull();}
...
if(this.gameTransitionTimer){var dt=2-(this.gameTransitionTimer.delta()*-1);this.menu.transition=(dt/2);var sy=ig.system.height-this.keyboard.height*this.keyboard.drawScale-30;var move=sy-MenuTitle.prototype.playerPos.y;this.menu.playerPos.y=ig.ease.inOutBack(dt,MenuTitle.prototype.playerPos.y,move,2);this.menu.alpha=1-(dt/2);this.player.pos.y=this.menu.playerPos.y;if(dt>2){this.gameTransitionTimer=null;this.menu=null;}}}
...
var resize=function(){height=Math.min((window.innerHeight/(window.innerWidth))*width,852);canvas.style.position='absolute';canvas.style.display='block';canvas.style.width=window.innerWidth+'px';canvas.style.height=(window.innerWidth/width)*height+'px';canvas.style.bottom='auto';canvas.style.right='auto';if(ig.game&&ig.system){ig.system.resize(width,height);}}
